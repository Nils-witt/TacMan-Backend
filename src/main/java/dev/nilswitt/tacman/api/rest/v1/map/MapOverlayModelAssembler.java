...
public class MapOverlayModelAssembler
        implements
        RepresentationModelAssembler<MapOverlayDto, EntityModel<MapOverlayDto>> {
    ...
    public EntityModel<MapOverlayDto> toModel(MapOverlayDto mapItem) {
        return EntityModel.of(
                ...
                WebMvcLinkBuilder.linkTo(
                        methodOn(MapOverlayController.class).one(mapItem.getId(), null)
                ).withSelfRel(),
                linkTo(methodOn(MapOverlayController.class).all(null)).withRel(
                        "map/items"
                )
        );
    }
