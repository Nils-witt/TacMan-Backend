...
public class SecurityGroupController {
    ...
    EntityModel<SecurityGroupDto> newEntity(
        ...
        group.setSsoGroupName(newEntity.getSsoGroupName() != null ? newEntity.getSsoGroupName() : "");
    ...
    EntityModel<SecurityGroupDto> one(@PathVariable UUID id, @AuthenticationPrincipal User userDetails) {
        SecurityGroup entity = this.repository.findById(id).orElseThrow(() -> new SecurityGroupNotFoundException(id));
    ...
    EntityModel<SecurityGroupDto> replaceEntity(
        ...
        entity.setSsoGroupName(newEntity.getSsoGroupName() != null ? newEntity.getSsoGroupName() : "");
        if (newEntity.getRoles() != null) {
    ...
    private Set<SecurityGroup.UserRoleScopeEnum> effectiveScopes(User user) {
        return EnumSet.allOf(SecurityGroup.UserRoleScopeEnum.class)
            .stream()
            .filter(scope ->
                this.permissionVerifier.hasAccess(user, scope, SecurityGroup.UserRoleTypeEnum.SECURITYGROUP)
            )
