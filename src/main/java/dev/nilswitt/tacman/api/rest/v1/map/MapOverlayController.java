...
public class MapOverlayController {
  ...
  @GetMapping("")
  CollectionModel<EntityModel<MapOverlayDto>> all(
    ...
    if (
      ...
      List<EntityModel<MapOverlayDto>> entities = this.repository.findAll()
        ...
        .map(mapOverlay -> {
          ...
          dto.setPermissions(
            this.permissionVerifier.getScopes(mapOverlay, userDetails)
          );
          return dto;
        })
  ...
  @GetMapping("{id}")
  EntityModel<MapOverlayDto> one(
    @PathVariable UUID id,
    @AuthenticationPrincipal User userDetails
  ) {
    MapOverlay entity = this.repository.findById(id).orElseThrow(() ->
      new MapOverlayNotFoundException(id)
    );
    if (
      !this.permissionVerifier.hasAccess(
        userDetails,
