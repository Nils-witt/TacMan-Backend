...
public class MapGroupController {
    ...
    @GetMapping("")
    CollectionModel<EntityModel<MapGroupDto>> all(
        ...
        if (
            ...
            List<EntityModel<MapGroupDto>> entities = this.repository.findAll()
                    ...
                    .map(mapGroup -> {
                        ...
                        dto.setPermissions(
                                this.permissionVerifier.getScopes(mapGroup, userDetails)
                        );
                        return dto;
                    })
    ...
    @GetMapping("{id}")
    EntityModel<MapGroupDto> one(
            @PathVariable UUID id,
            @AuthenticationPrincipal User userDetails
    ) {
        MapGroup entity = this.repository.findById(id).orElseThrow(() ->
