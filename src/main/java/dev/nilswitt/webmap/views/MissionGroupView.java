...
import dev.nilswitt.webmap.views.components.MissionGroupEditDialog;
...
public class MissionGroupView extends VerticalLayout {
    ...
    public MissionGroupView(MissionGroupRepository missionRepository,
        ...
        Button createBtn = new Button("Create", e -> {
            ...
            if (!permissionUtil.hasAccess(user, SecurityGroup.UserRoleScopeEnum.CREATE, SecurityGroup.UserRoleTypeEnum.MISSIONGROUP)) {
    ...
    private class MissionContextMenu extends GridContextMenu<MissionGroup> {
        MissionContextMenu(Grid<MissionGroup> target) {
            ...
            addItem("Edit", e -> e.getItem().ifPresent(mission -> {
                User user = currentUser();
                if (!permissionUtil.hasAccess(user, SecurityGroup.UserRoleScopeEnum.EDIT, SecurityGroup.UserRoleTypeEnum.MISSIONGROUP)) {
            ...
            addItem("Delete", e -> e.getItem().ifPresent(mission -> {
                User user = currentUser();
                if (!permissionUtil.hasAccess(user, SecurityGroup.UserRoleScopeEnum.DELETE, SecurityGroup.UserRoleTypeEnum.MISSIONGROUP)) {
