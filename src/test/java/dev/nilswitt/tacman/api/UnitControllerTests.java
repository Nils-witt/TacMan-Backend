...
class UnitControllerTests {
  ...
  void shouldCreateUnit() throws Exception {
    ...
    MvcResult result = mockMvc
      .perform(
        post("/api/units").contentType(MediaType.APPLICATION_JSON).content(body)
      )
      .andExpect(status().isOk())
  ...
  void shouldGetUnitPositionHistory() throws Exception {
    ...
    mockMvc
      .perform(get("/api/units/" + id + "/position/history"))
      .andExpect(status().isOk());
  ...
  void shouldReturn404ForNonExistentUnit() throws Exception {
    ...
    mockMvc
      .perform(get("/api/units/" + randomId))
      .andExpect(status().isNotFound());

    mockMvc
      .perform(
        patch("/api/units/" + randomId)
          .contentType(MediaType.APPLICATION_JSON)
          .content("{}")
      )
      .andExpect(status().isNotFound());
    ...
    mockMvc
      .perform(delete("/api/units/" + randomId))
      .andExpect(status().isNotFound());
  }
}
