...
class UnitControllerTests {
    ...
    void shouldCreateUnit() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/units")
    ...
    void shouldGetUnitById() throws Exception {
        ...
        MvcResult created = mockMvc.perform(post("/api/units")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
            .andExpect(status().isOk())
            .andReturn();
    ...
    void shouldPatchUnit() throws Exception {
        ...
        mockMvc.perform(patch("/api/units/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                    {"name": "Unit_%s"}
                    """.formatted(unique2)))
    ...
    void shouldDeleteUnit() throws Exception {
        ...
        mockMvc.perform(delete("/api/units/" + id))
            .andExpect(status().isOk());
    ...
    void shouldGetUnitStatusHistory() throws Exception {
        ...
        mockMvc.perform(get("/api/units/" + id + "/status/history"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$._links.self").exists());
    ...
    void shouldGetUnitPositionHistory() throws Exception {
        ...
        mockMvc.perform(get("/api/units/" + id + "/position/history"))
            .andExpect(status().isOk());
    ...
    void shouldReturn404ForNonExistentUnit() throws Exception {
        ...
        mockMvc.perform(get("/api/units/" + randomId))
            .andExpect(status().isNotFound());

        mockMvc.perform(patch("/api/units/" + randomId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}"))
            .andExpect(status().isNotFound());

        mockMvc.perform(delete("/api/units/" + randomId))
            .andExpect(status().isNotFound());
    }
}
