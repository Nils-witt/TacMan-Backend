...
class ApiControllerTests {
    ...
    private MockHttpServletRequestBuilder buildRequest(String method, String path) {
        return switch (method) {
            ...
            case "PATCH" -> patch(path).contentType(MediaType.APPLICATION_JSON).content("{}");
    ...
    void shouldIssueAndValidateTokenForAdmin() throws Exception {
        ...
        MvcResult obtainResult = mockMvc
            .perform(post("/api/token").contentType(MediaType.APPLICATION_JSON).content(body))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.token").isNotEmpty())
            .andExpect(jsonPath("$.userId").isNotEmpty())
            .andReturn();

        String token = JsonPath.read(obtainResult.getResponse().getContentAsString(StandardCharsets.UTF_8), "$.token");
    ...
    void shouldRequireAuthenticationForAllSecuredEndpoints(String method, String path) throws Exception {
        mockMvc.perform(buildRequest(method, path)).andExpect(status().isUnauthorized());
    }
