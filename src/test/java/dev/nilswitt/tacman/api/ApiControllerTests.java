...
class ApiControllerTests {
    ...
    void shouldIssueAndValidateTokenForAdmin() throws Exception {
        ...
        MvcResult obtainResult = mockMvc.perform(post("/api/token")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
            .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/token").header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
    ...
    void shouldRequireAuthenticationForAllSecuredEndpoints(String method, String path) throws Exception {
        mockMvc.perform(buildRequest(method, path))
            .andExpect(status().isUnauthorized());
    }
