...
class MissionGroupControllerTests {
    ...
    void shouldListMissionGroups() throws Exception {
        mockMvc.perform(get("/api/missiongroups"))
    ...
    void shouldCreateMissionGroup() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/missiongroups")
    ...
    void shouldGetMissionGroupById() throws Exception {
        ...
        MvcResult created = mockMvc.perform(post("/api/missiongroups")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc.perform(get("/api/missiongroups/" + id))
            .andExpect(status().isOk())
    ...
    void shouldUpdateMissionGroup() throws Exception {
        ...
        mockMvc.perform(put("/api/missiongroups/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(updateBody))
            .andExpect(status().isOk())
    ...
    void shouldDeleteMissionGroup() throws Exception {
        ...
        mockMvc.perform(delete("/api/missiongroups/" + id))
            .andExpect(status().isOk());
    ...
    void shouldReturn404ForNonExistentMissionGroup() throws Exception {
        ...
        mockMvc.perform(get("/api/missiongroups/" + randomId))
            .andExpect(status().isNotFound());

        mockMvc.perform(put("/api/missiongroups/" + randomId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
        ...
        mockMvc.perform(delete("/api/missiongroups/" + randomId))
            .andExpect(status().isNotFound());
    }
}
