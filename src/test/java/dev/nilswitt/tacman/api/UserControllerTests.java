...
class UserControllerTests {
    ...
    void shouldListUsers() throws Exception {
        mockMvc.perform(get("/api/users")).andExpect(status().isOk()).andExpect(jsonPath("$._links.self").exists());
    }
    ...
    void shouldUpdateUser() throws Exception {
        ...
        MvcResult created = mockMvc
            .perform(post("/api/users").contentType(MediaType.APPLICATION_JSON).content(createBody))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc
            .perform(put("/api/users/" + id).contentType(MediaType.APPLICATION_JSON).content(updateBody))
            .andExpect(status().isOk())
    ...
    void shouldReturn404ForNonExistentUser() throws Exception {
        ...
        mockMvc.perform(get("/api/users/" + randomId)).andExpect(status().isNotFound());

        mockMvc
            .perform(put("/api/users/" + randomId).contentType(MediaType.APPLICATION_JSON).content("{}"))
            .andExpect(status().isNotFound());

        mockMvc.perform(delete("/api/users/" + randomId)).andExpect(status().isNotFound());
    }
}
