...
class MapBaseLayerControllerTests {
    ...
    void shouldCreateBaseLayer() throws Exception {
        ...
        MvcResult result = mockMvc
            .perform(post("/api/map/baselayers").contentType(MediaType.APPLICATION_JSON).content(body))
            .andExpect(status().isOk())
        ...
        // cleanup
        mockMvc.perform(delete("/api/map/baselayers/" + id)).andExpect(status().isOk());
    }
    ...
    void shouldUpdateBaseLayer() throws Exception {
        ...
        MvcResult created = mockMvc
            .perform(post("/api/map/baselayers").contentType(MediaType.APPLICATION_JSON).content(createBody))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc
            .perform(put("/api/map/baselayers/" + id).contentType(MediaType.APPLICATION_JSON).content(updateBody))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.name").value("Layer_" + unique + "_updated"))
            .andExpect(jsonPath("$.url").value("https://updated.example.com/{z}/{x}/{y}.png"));
    ...
    void shouldDeleteBaseLayer() throws Exception {
        ...
        mockMvc.perform(get("/api/map/baselayers/" + id)).andExpect(status().isNotFound());
    }
    ...
    void shouldReturn404ForNonExistentBaseLayer() throws Exception {
        ...
        mockMvc.perform(get("/api/map/baselayers/" + randomId)).andExpect(status().isNotFound());
        ...
        mockMvc.perform(delete("/api/map/baselayers/" + randomId)).andExpect(status().isNotFound());
    }
}
