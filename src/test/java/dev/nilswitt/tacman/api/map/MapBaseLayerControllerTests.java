...
class MapBaseLayerControllerTests {
    ...
    void shouldListBaseLayers() throws Exception {
        mockMvc.perform(get("/api/map/baselayers"))
    ...
    void shouldCreateBaseLayer() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/map/baselayers")
    ...
    void shouldGetBaseLayerById() throws Exception {
        ...
        MvcResult created = mockMvc.perform(post("/api/map/baselayers")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc.perform(get("/api/map/baselayers/" + id))
            .andExpect(status().isOk())
    ...
    void shouldUpdateBaseLayer() throws Exception {
        ...
        mockMvc.perform(put("/api/map/baselayers/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(updateBody))
            .andExpect(status().isOk())
    ...
    void shouldDeleteBaseLayer() throws Exception {
        ...
        mockMvc.perform(delete("/api/map/baselayers/" + id))
            .andExpect(status().isOk());
    ...
    void shouldReturn404ForNonExistentBaseLayer() throws Exception {
        ...
        mockMvc.perform(get("/api/map/baselayers/" + randomId))
            .andExpect(status().isNotFound());

        mockMvc.perform(put("/api/map/baselayers/" + randomId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
        ...
        mockMvc.perform(delete("/api/map/baselayers/" + randomId))
            .andExpect(status().isNotFound());
    }
}
