...
class MapItemControllerTests {
    ...
    void shouldListMapItems() throws Exception {
        mockMvc.perform(get("/api/map/items"))
    ...
    void shouldCreateMapItem() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/map/items")
    ...
    void shouldGetMapItemById() throws Exception {
        ...
        MvcResult created = mockMvc.perform(post("/api/map/items")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc.perform(get("/api/map/items/" + id))
            .andExpect(status().isOk())
    ...
    void shouldUpdateMapItem() throws Exception {
        ...
        mockMvc.perform(put("/api/map/items/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(updateBody))
            .andExpect(status().isOk())
    ...
    void shouldDeleteMapItem() throws Exception {
        ...
        mockMvc.perform(delete("/api/map/items/" + id))
            .andExpect(status().isOk());
    ...
    void shouldReturn404ForNonExistentMapItem() throws Exception {
        ...
        mockMvc.perform(get("/api/map/items/" + randomId))
            .andExpect(status().isNotFound());

        mockMvc.perform(put("/api/map/items/" + randomId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
        ...
        mockMvc.perform(delete("/api/map/items/" + randomId))
            .andExpect(status().isNotFound());
    }
}
