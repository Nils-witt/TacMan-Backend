...
class MapItemControllerTests {
    ...
    void shouldListMapItems() throws Exception {
        mockMvc.perform(get("/api/map/items")).andExpect(status().isOk()).andExpect(jsonPath("$._links.self").exists());
    }
    ...
    void shouldCreateMapItem() throws Exception {
        ...
        MvcResult result = mockMvc
            .perform(post("/api/map/items").contentType(MediaType.APPLICATION_JSON).content(body))
    ...
    void shouldUpdateMapItem() throws Exception {
        ...
        MvcResult created = mockMvc
            .perform(post("/api/map/items").contentType(MediaType.APPLICATION_JSON).content(createBody))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc
            .perform(put("/api/map/items/" + id).contentType(MediaType.APPLICATION_JSON).content(updateBody))
            .andExpect(status().isOk())
    ...
    void shouldDeleteMapItem() throws Exception {
        ...
        mockMvc.perform(get("/api/map/items/" + id)).andExpect(status().isNotFound());
    }
    ...
    void shouldReturn404ForNonExistentMapItem() throws Exception {
        ...
        mockMvc.perform(get("/api/map/items/" + randomId)).andExpect(status().isNotFound());
        ...
        mockMvc.perform(delete("/api/map/items/" + randomId)).andExpect(status().isNotFound());
    }
}
