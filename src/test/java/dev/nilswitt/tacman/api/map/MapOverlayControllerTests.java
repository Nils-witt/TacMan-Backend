...
class MapOverlayControllerTests {
    ...
    void shouldListOverlays() throws Exception {
        mockMvc.perform(get("/api/map/overlays"))
    ...
    void shouldCreateOverlay() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/map/overlays")
    ...
    void shouldGetOverlayById() throws Exception {
        ...
        MvcResult created = mockMvc.perform(post("/api/map/overlays")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
            .andExpect(status().isOk())
            .andReturn();
        ...
        mockMvc.perform(get("/api/map/overlays/" + id))
            .andExpect(status().isOk())
    ...
    void shouldUpdateOverlay() throws Exception {
        ...
        mockMvc.perform(put("/api/map/overlays/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(updateBody))
            .andExpect(status().isOk())
    ...
    void shouldDeleteOverlay() throws Exception {
        ...
        mockMvc.perform(delete("/api/map/overlays/" + id))
            .andExpect(status().isOk());
    ...
    void shouldReturn404ForNonExistentOverlay() throws Exception {
        ...
        mockMvc.perform(get("/api/map/overlays/" + randomId))
            .andExpect(status().isNotFound());

        mockMvc.perform(put("/api/map/overlays/" + randomId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
        ...
        mockMvc.perform(delete("/api/map/overlays/" + randomId))
            .andExpect(status().isNotFound());
    }
}
